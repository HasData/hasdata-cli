...
func newAirbnbListingCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&p_checkInVar, "check-in", "2026-10-19", "checkIn Check-in Date: The check-in date for the listings. (required)")
	_ = cmd.MarkFlagRequired("check-in")
	cmd.Flags().StringVar(&p_checkOutVar, "check-out", "2026-10-23", "checkOut Check-out Date: The check-out date for the listings.")
	...
	cmd.Flags().StringVar(&p_locationVar, "location", "New York", "location Location: The location to search for listings. Required unless a full map bounding box (neLat, neLng, swLat, swLng) is provided.")
